...
router.get('/root/character/:characterID', async (req, res, next) => {
  try {
    ...
    const rootInteraction = await Interaction.findOne({
      where: {
...
router.get('/node/:interactionId', async (req, res, next) => {
  try {
